...
from .recommender import create_user_item_matrix, apply_svd, calculate_similarity, recommend_products
from django.conf import settings

def recommend(request):
    all_entries = MyList.objects.all().values_list('user_id', 'product_id', 'paid_status')
    data = [{'user_id': entry[0], 'product_id': entry[1], 'paid_status':entry[2]} for entry in all_entries]

    user_item_matrix = create_user_item_matrix(data)
    print("USER_ITEM_MATRIX", user_item_matrix)
    reduced_matrix = apply_svd(user_item_matrix)
    print("REDUSED_MATRIX", reduced_matrix)
    user_similarity_matrix = calculate_similarity(reduced_matrix)
    print("USER_SIMILARITY_MATRIX", user_similarity_matrix)

    user_id = request.user.id
    top_n = request.GET.get('top_n', settings.DEFAULT_TOP_N)
    similarity_threshold = request.GET.get('similarity_threshold', settings.DEFAULT_SIMILARITY_THRESHOLD)

    recommended_product_ids = []
    if user_id in user_item_matrix.index:
        # Example of fetching product data with categories
        product_data = {product.id: {'category': product.category_id} for product in Product.objects.all()}
        recommended_product_ids = recommend_products(user_id, user_similarity_matrix, user_item_matrix, product_data, int(top_n), float(similarity_threshold))
        print("REC: ", recommended_product_ids)

    recommended_products = Product.objects.filter(id__in=recommended_product_ids)
    count = recommended_products.count()

    return render(request, 'core/recommend.html', {'recommended_products': recommended_products, "count": count})
...
def add_to_cart(request):
    ...
    if user is not None:
        ...
        product, created = Product.objects.get_or_create(id=product_id)

        myList_entry, created = MyList.objects.get_or_create(user=user, product=product, defaults={'liked': True})

        if created:
            # The entry didn't exist and was created, you can perform any additional setup here
            pass
    ...
    if request.method=="POST":
        ...
        for p_id, item in request.session['cart_data_object'].items():
            ...
            if user is not None:
                ...
                product, created = Product.objects.get_or_create(pid=product_pid)
...
def add_to_wishlist(request):
    ...
    context = {}
    # If user is anonymous, return an instruction to log in
